...
@routes.post("/admin/audit-bin")
async def admin_audit_bin(request: web.Request) -> web.Response:
    """Kick off a BIN_CHANNEL <-> catalogue reconciliation walk.

    Background task — progress shown on the existing admin progress
    widget under the ``audit`` key. Once finished, the report is
    visible at GET /admin/audit-bin.
    """
    _require_session(request)
    if not media_index.audit_state().get("running"):
        import asyncio as _aio
        _aio.create_task(media_index.audit_bin(StreamBot, Var.BIN_CHANNEL))
    if _is_htmx(request):
        return web.Response(status=204)
    raise web.HTTPFound("/admin/audit-bin")


@routes.get("/admin/audit-bin")
async def admin_audit_bin_view(request: web.Request) -> web.Response:
    """Render the last audit report as a separate page with per-row
    fix buttons."""
    _require_session(request)
    # Read flash cookie (set by the fix endpoints when they redirect
    # back here) so the toast renders + clears on view.
    raw = request.cookies.get(_FLASH_COOKIE) or ""
    flash = ""
    if raw:
        from urllib.parse import unquote as _u
        try:
            ...
            flash = ""
    tpl = _env.get_template("audit_bin.html")
    body = await tpl.render_async(
        report=media_index.audit_report(),
        state=media_index.audit_state(),
        flash=flash,
    )
    resp = _html(body)
    if raw:
...
@routes.post("/admin/audit-bin/reindex")
async def admin_audit_reindex(request: web.Request) -> web.Response:
    """Re-index a BIN orphan (file in BIN, no catalogue entry).

    Accepts one of:
      • ``id=<message_id>`` — re-index a single message
      • ``all=1``           — re-index every BIN orphan in the
                              current report
    """
    _require_session(request)
    form = await request.post()
    ...
    if not report.get("ready"):
        raise _redirect_with_flash("No audit report — run the audit first.",
                                   target="/admin/audit-bin")
    targets: List[int] = []
    if form.get("all"):
        targets = [int(o["message_id"]) for o in report.get("bin_orphans") or []]
    elif form.get("id"):
        try:
            targets = [int(form.get("id"))]
        except (TypeError, ValueError):
            targets = []
    if not targets:
        raise _redirect_with_flash("Nothing to re-index.",
                                   target="/admin/audit-bin")
    ...
    for mid in targets:
        try:
            ...
            await media_index.add_from_message(msg)
            fixed += 1
        except Exception:
            logging.exception("audit reindex failed for bin:%d", mid)

    # Drop the re-indexed entries from the report so the page reflects
    # the new state without re-running the audit.
    remaining = [o for o in (report.get("bin_orphans") or [])
                 if int(o["message_id"]) not in set(targets)]
    media_index._audit_report["bin_orphans"] = remaining

    raise _redirect_with_flash(
        f"Re-indexed {fixed} BIN orphan{'' if fixed == 1 else 's'}.",
        target="/admin/audit-bin",
    )


@routes.post("/admin/audit-bin/remove")
async def admin_audit_remove(request: web.Request) -> web.Response:
    """Drop a catalogue orphan (no underlying BIN message) from the
    catalogue. Accepts ``id=<message_id>`` or ``all=1``.
    """
    _require_session(request)
    form = await request.post()
    report = media_index.audit_report()
    ...
    targets: List[int] = []
    if form.get("all"):
        targets = [int(o["message_id"]) for o in report.get("catalogue_orphans") or []]
    elif form.get("id"):
        try:
            targets = [int(form.get("id"))]
        except (TypeError, ValueError):
            targets = []
    if not targets:
        raise _redirect_with_flash("Nothing to remove.",
                                   target="/admin/audit-bin")
    ...
    for mid in targets:
        try:
            ...
            logging.exception("audit remove failed for bin:%d", mid)

    remaining = [o for o in (report.get("catalogue_orphans") or [])
                 if int(o["message_id"]) not in set(targets)]
    media_index._audit_report["catalogue_orphans"] = remaining

    raise _redirect_with_flash(
        f"Removed {removed} catalogue orphan{'' if removed == 1 else 's'}.",
        target="/admin/audit-bin",
    )
