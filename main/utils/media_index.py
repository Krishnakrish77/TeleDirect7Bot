...
# BIN <-> catalogue reconciliation report. Built by audit_bin().
# Stored in memory; the admin "Audit BIN" page renders it.
_audit_state: dict = {
...
_audit_report: dict = {
    "ready": False,
    "ran_at": 0.0,
    "bin_orphans": [],          # message dicts present in BIN, missing from catalogue
    "catalogue_orphans": [],    # HubItem dicts present in catalogue, missing from BIN
    "total_bin_scanned": 0,
    "total_catalogue": 0,
}
...
def audit_report() -> dict:
    """Return the most recent audit report (or an empty stub if no
    audit has been run yet).
    """
    return dict(_audit_report)


async def audit_bin(bot, channel_id: int) -> dict:
    """Walk BIN_CHANNEL backward from latest down to floor and reconcile
    against the in-memory catalogue.

    Produces a read-only report listing:
      • BIN orphans — media messages present in BIN that don't appear
        in the catalogue. These are uploads that were probably deleted
        from the catalogue by a buggy dedup pass, or that failed to
        index originally. Admin can re-index them via add_from_message.
      • Catalogue orphans — catalogue rows whose underlying BIN message
        no longer exists (deleted in Telegram, not yet pruned). The
        startup stale-prune sweep already removes these, but a manual
        audit may find some between restarts.

    No mutations — the report just records what's reconcilable; admin
    decides what to do with the findings.
    """
    if _audit_state["running"]:
        return {"already_running": True}

    # We walk up to _SEED_DEPTH below latest_id, same window as seed().
    ...
    if latest_id <= 0:
        try:
            ...
            logging.exception("audit_bin: probe failed")
            return {"error": "probe failed"}

    floor = max(1, latest_id - _SEED_DEPTH)
    total_to_scan = latest_id - floor + 1

    _audit_state.update(
        running=True, done=0, total=total_to_scan,
        started_at=time.time(), finished_at=0.0,
    )

    bin_message_ids: set = set()
    bin_media_info: dict = {}   # message_id -> {file_name, file_size, mime, date}
    high = latest_id
    ...
    try:
        while high >= floor:
            ...
            try:
                ...
                logging.exception(
                    "audit_bin: get_messages failed %d..%d", ids[-1], ids[0],
                )
                break
            if not isinstance(batch, list):
                batch = [batch]
            for m in batch:
                if m is None or getattr(m, "empty", False):
                    continue
                bin_message_ids.add(int(m.id))
                # Capture media info for the orphan report.
                if _is_video_message(m):
                    media = _media_of(m)
                    bin_media_info[int(m.id)] = {
                        "message_id": int(m.id),
            ...
            _audit_state["done"] = min(total_to_scan, latest_id - high + _FETCH_BATCH)
            high -= _FETCH_BATCH
            await asyncio.sleep(0)
    finally:
        _audit_state["running"] = False
        _audit_state["finished_at"] = time.time()

    catalogue_ids = set(_items.keys())
    bin_orphans_ids = sorted(
        (mid for mid in bin_media_info.keys() if mid not in catalogue_ids),
        reverse=True,
    )
    bin_orphans = [bin_media_info[mid] for mid in bin_orphans_ids]
    catalogue_orphans = []
    for mid in sorted(catalogue_ids, reverse=True):
        if mid < floor or mid > latest_id:
            # Outside walked range — can't audit.
            continue
        if mid not in bin_message_ids:
            it = _items[mid]
            catalogue_orphans.append({
                "message_id": mid,
                "title": it.title,
                "year": it.year,
    ...
    # Stringify dates so the report is JSON-serialisable for the
    # template (we render via Jinja, which prefers primitives).
    for entry in bin_orphans:
        d = entry.get("date")
        entry["date"] = d.isoformat() if hasattr(d, "isoformat") else (str(d) if d else "")

    _audit_report.update(
        ready=True,
        ran_at=time.time(),
        bin_orphans=bin_orphans,
        catalogue_orphans=catalogue_orphans,
        total_bin_scanned=len(bin_message_ids),
        total_catalogue=len(catalogue_ids),
        floor=floor,
        latest_id=latest_id,
    )
    logging.info(
        "audit_bin: %d BIN messages scanned, %d orphans, %d catalogue orphans",
        len(bin_message_ids), len(bin_orphans), len(catalogue_orphans),
    )
    return {
        "bin_scanned": len(bin_message_ids),
        "bin_orphans": len(bin_orphans),
        "catalogue_orphans": len(catalogue_orphans),
    }
