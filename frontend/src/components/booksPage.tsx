...
export function BooksPage() {
  ...
  const [bookmarks, setBookmarks] = useState<Record<string, BookBookmark[]>>(() => localBookmarks()); const [toc, setToc] = useState<EpubTocItem[]>([]); const [readerPanel, setReaderPanel] = useState<'contents' | 'bookmarks' | null>(null); const [findQuery, setFindQuery] = useState(''); const [findStatus, setFindStatus] = useState('');
  ...
  if (selected) return <main className="books-reader-page"><section className="books-reader-shell"><header className="books-reader-toolbar"><Button variant="ghost" size="sm" onClick={closeReader}>← Library</Button><div className="books-file-label"><BookOpenIcon /><span><strong>{selected.title}</strong><small>{selected.fileName} · {selected.fileSizeLabel}</small></span></div><div className="books-reader-actions">{isEpub && <><Button variant="secondary" size="sm" onClick={() => setReaderPanel(readerPanel === 'contents' ? null : 'contents')}>Contents</Button><Button variant="secondary" size="sm" onClick={addBookmark}>Bookmark</Button><Button variant="secondary" size="sm" onClick={() => setReaderPanel(readerPanel === 'bookmarks' ? null : 'bookmarks')}>Bookmarks</Button><Button variant="secondary" size="sm" onClick={() => void renditionRef.current?.prev()}>Previous</Button><Button variant="secondary" size="sm" onClick={() => void renditionRef.current?.next()}>Next</Button><Button variant="secondary" size="sm" onClick={speak}>{speaking ? <PauseIcon /> : <VolumeIcon />}{speaking ? 'Stop reading' : 'Listen'}</Button></>}<a href={selected.downloadUrl}><Button variant="secondary" size="sm"><DownloadIcon /> Download</Button></a></div></header>{readerError && <p className="books-reader-error" role="alert">{readerError}</p>}{isEpub && <div className="books-reader-find"><input value={findQuery} onChange={(event) => setFindQuery(event.currentTarget.value)} onKeyDown={(event) => { if (event.key === 'Enter') findInBook(); }} placeholder="Find in open chapter" /><Button variant="secondary" size="sm" onClick={findInBook}>Find</Button>{findStatus && <small>{findStatus}</small>}</div>}{readerPanel && isEpub && <aside className="books-reader-panel"><strong>{readerPanel === 'contents' ? 'Contents' : 'Bookmarks'}</strong>{readerPanel === 'contents' ? (toc.length ? renderToc(toc) : <small>Contents unavailable for this EPUB.</small>) : ((bookmarks[selected.id] || []).length ? (bookmarks[selected.id] || []).map((entry) => <button key={entry.t} type="button" className="books-reader-panel-link" onClick={() => { void renditionRef.current?.display(entry.locator); setReaderPanel(null); }}>{entry.label}</button>) : <small>No bookmarks yet.</small>)}</aside>}<div className="books-reader">{isEpub ? <div ref={epubRootRef} className="epub-reader" aria-label={`${selected.title} reader`} /> : <iframe title={selected.title} src={selected.readUrl} />}</div><footer className="books-reader-footer"><span>{isEpub && progress[selected.id]?.progress ? `${Math.round(progress[selected.id].progress * 100)}% read` : isEpub ? 'Starting chapter' : 'PDF reading controls'}</span><span>{isEpub ? 'Your place is saved automatically; bookmarks stay on this device.' : 'PDF page resume arrives with the PDF reader upgrade'}</span></footer></section></main>;
