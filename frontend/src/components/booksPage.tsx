...
type EpubRendition = { display: (target?: string | number) => Promise<unknown>; next: () => Promise<unknown>; prev: () => Promise<unknown>; on?: (event: string, callback: (location: EpubLocation) => void) => void; getContents?: () => Array<{ document?: Document }>; themes?: { register: (name: string, rules: Record<string, Record<string, string>>) => void; select: (name: string) => void }; annotations?: { highlight: (cfiRange: string, data?: unknown, callback?: unknown, className?: string, styles?: Record<string, string>) => void; remove?: (cfiRange: string, type?: string) => void }; destroy?: () => void; };
...
type PdfViewport = { width: number; height: number; scale: number; rotation: number; rawDims: { pageWidth: number; pageHeight: number; pageX: number; pageY: number } };
...
function isEditableElement(target: EventTarget | null): boolean { return target instanceof HTMLElement && (target.isContentEditable || ['INPUT', 'SELECT', 'TEXTAREA'].includes(target.tagName)); }
function browserEpubReader() { return (window as unknown as { ePub?: NonNullable<Window['ePub']> }).ePub; }
function browserZipReader() { return (window as unknown as { JSZip?: JsZipStatic }).JSZip; }
function loadEpubReader(): Promise<void> {
  if (browserEpubReader() && browserZipReader()) return Promise.resolve();
  if (!epubReaderLoad) epubReaderLoad = Promise.all([import('epubjs'), import('jszip')]).then(([epubModule, zipModule]) => {
    epubReader = epubModule.default as unknown as NonNullable<Window['ePub']>;
    zipReader = zipModule.default as unknown as JsZipStatic;
  });
  return epubReaderLoad;
}
...
export function BooksPage({ user }: { user: User | null }) {
  const [items, setItems] = useState<BookItem[]>([]); const [bookTotal, setBookTotal] = useState(0); const [nextBookOffset, setNextBookOffset] = useState<number | null>(null); const [loadingMoreBooks, setLoadingMoreBooks] = useState(false); const [query, setQuery] = useState(''); const [debouncedQuery, setDebouncedQuery] = useState(''); const [selected, setSelected] = useState<BookItem | null>(null);
  ...
  useEffect(() => {
    ...
    if (!engine?.getVoices) return undefined;
    const loadVoices = () => setSpeechVoices(engine.getVoices());
    loadVoices(); engine.addEventListener?.('voiceschanged', loadVoices);
    return () => engine.removeEventListener?.('voiceschanged', loadVoices);
  }, []);
  ...
  useEffect(() => {
    ...
    void loadEpubReader().then(() => epubSource(selected.readUrl, selected.fileSize)).then((source) => {
      ...
      epubRootRef.current.replaceChildren(); book = ePub(source, { openAs: 'binary' }); epubBookRef.current = book;
      ...
      const labelEpubFrames = () => epubRootRef.current?.querySelectorAll('iframe').forEach((frame, index) => { frame.title = `${selected.title}, reading page ${index + 1}`; });
      const bindEpubTouch = () => rendition.getContents?.().forEach((content) => {
        const doc = content.document; if (!doc || boundEpubDocuments.has(doc)) return; boundEpubDocuments.add(doc);
        let start: { x: number; y: number } | null = null; let skipClick = false;
        const showControls = () => { if (!readerMenuOpenRef.current) setControlsVisible(true); };
        const turnAtEdge = (x: number) => {
          if (readerMenuOpenRef.current) return false;
          const width = doc.documentElement.clientWidth || window.innerWidth;
          if (x < width * .25) { setControlsVisible(true); void rendition.prev(); return true; }
          if (x > width * .75) { setControlsVisible(true); void rendition.next(); return true; }
          return false;
        };
        const onClick = (event: MouseEvent) => { if (skipClick) { skipClick = false; return; } if (!turnAtEdge(event.clientX)) showControls(); };
        const onStart = (event: TouchEvent) => { const touch = event.changedTouches[0]; if (touch) start = { x: touch.clientX, y: touch.clientY }; };
        const onEnd = (event: TouchEvent) => {
          const touch = event.changedTouches[0]; if (!start || !touch || readerMenuOpenRef.current) return;
          const dx = touch.clientX - start.x; const dy = touch.clientY - start.y; start = null; skipClick = true;
          if (Math.abs(dx) >= 56 && Math.abs(dy) <= Math.abs(dx)) { setControlsVisible(true); void (dx < 0 ? rendition.next() : rendition.prev()); return; }
          if (Math.abs(dy) <= 28 && turnAtEdge(touch.clientX)) return;
          showControls();
        };
        doc.addEventListener('click', onClick); doc.addEventListener('touchstart', onStart, { passive: true }); doc.addEventListener('touchend', onEnd, { passive: true });
        epubTouchCleanups.push(() => { doc.removeEventListener('click', onClick); doc.removeEventListener('touchstart', onStart); doc.removeEventListener('touchend', onEnd); });
      });
      rendition.on?.('rendered', () => { bindEpubTouch(); labelEpubFrames(); });
      ...
      return rendition.display(progress[selected.id]?.locator || undefined).then((result) => { window.clearTimeout(timeout); if (!cancelled) { setReaderLoading(false); setReaderError(''); } bindEpubTouch(); labelEpubFrames(); return result; });
    ...
    return () => { cancelled = true; epubSearchTokenRef.current += 1; const highlighted = epubSearchHighlightRef.current; if (highlighted) renditionRef.current?.annotations?.remove?.(highlighted, 'highlight'); epubSearchHighlightRef.current = null; window.clearTimeout(timeout); epubTouchCleanups.forEach((cleanup) => cleanup()); renditionRef.current?.destroy?.(); renditionRef.current = null; epubBookRef.current = null; book?.destroy?.(); };
  ...
  useEffect(() => {
    ...
    let cancelled = false; let renderTask: { promise: Promise<unknown>; cancel?: () => void } | null = null; let textTask: { render: () => Promise<unknown>; cancel?: () => void } | null = null;
    void pdfDocument.getPage(Math.max(1, Math.min(pdfPage, pdfDocument.numPages))).then((page) => {
      ...
      const textLayer = pdfTextLayerRef.current; const TextLayer = pdfJsRef.current?.TextLayer;
      if (!textLayer || !TextLayer) return renderTask.promise;
      textLayer.replaceChildren(); textLayer.style.width = `${Math.ceil(displayViewport.width)}px`; textLayer.style.height = `${Math.ceil(displayViewport.height)}px`;
      return page.getTextContent().then((textContent) => {
        pdfTextCacheRef.current.set(pdfPage, textContent.items.map((item) => item.str || '').join(' ').toLocaleLowerCase());
        if (cancelled) return renderTask?.promise;
        textTask = new TextLayer({ textContentSource: textContent, container: textLayer, viewport: displayViewport });
        return Promise.all([renderTask?.promise, textTask.render()]).then(() => {
          const query = pdfHighlightQuery;
          if (query) textLayer.querySelectorAll('span').forEach((span) => span.classList.toggle('is-search-hit', (span.textContent || '').toLocaleLowerCase().includes(query)));
        });
      });
    }).then(() => {
  ...
  }, [pdfDocument, pdfPage, pdfReaderWidth, pdfZoom, pdfHighlightQuery]);

  const stopSpeech = () => { speechTokenRef.current += 1; speechChunksRef.current = []; speechChunkIndexRef.current = 0; speechOnCompleteRef.current = undefined; window.speechSynthesis?.cancel(); setSpeaking(false); setSpeechPaused(false); setSpeechPage(null); };
  const playSpeech = (chunks: string[], page: number | null, onComplete?: () => void, startIndex = 0) => {
    if (!('speechSynthesis' in window)) { setReaderError('Read aloud is not available in this browser.'); return; }
    const token = speechTokenRef.current + 1; speechTokenRef.current = token; speechChunksRef.current = chunks; speechChunkIndexRef.current = startIndex; speechOnCompleteRef.current = onComplete; window.speechSynthesis.cancel(); setReaderError(''); setSpeaking(true); setSpeechPaused(false); setSpeechPage(page);
    const playChunk = (index: number) => {
      if (token !== speechTokenRef.current) return;
      if (index >= chunks.length) { speechChunksRef.current = []; speechChunkIndexRef.current = 0; speechOnCompleteRef.current = undefined; if (onComplete) { onComplete(); return; } setSpeaking(false); setSpeechPaused(false); setSpeechPage(null); return; }
      speechChunkIndexRef.current = index;
      const utterance = new SpeechSynthesisUtterance(chunks[index]); utterance.rate = speechRate;
      const voice = speechVoices.find((entry) => entry.voiceURI === speechVoiceUri);
  ...
  useEffect(() => {
    const previous = speechSettingsRef.current; speechSettingsRef.current = { rate: speechRate, voiceUri: speechVoiceUri };
    if (!speaking || (previous.rate === speechRate && previous.voiceUri === speechVoiceUri) || !speechChunksRef.current.length) return;
    playSpeech(speechChunksRef.current, speechPage, speechOnCompleteRef.current, speechChunkIndexRef.current);
  ...
  useEffect(() => { if (!selected) return undefined; const onKey = (event: KeyboardEvent) => {
    if (event.key === 'Escape') { event.preventDefault(); setReaderMenuOpen(false); setReaderPanel(null); setControlsVisible(true); return; }
    if (isEditableElement(event.target)) return;
    if (event.key === '?' || (event.key === '/' && event.shiftKey)) { event.preventDefault(); setControlsVisible(true); setReaderMenuOpen(true); return; }
    if (event.key === 'ArrowLeft') { event.preventDefault(); turnPage(-1); }
    if (event.key === 'ArrowRight') { event.preventDefault(); turnPage(1); }
  }; window.addEventListener('keydown', onKey); return () => window.removeEventListener('keydown', onKey); }, [selected, isEpub, pdfPage]);
  useEffect(() => { if (!selected) return undefined; const down = (event: PointerEvent) => { const target = event.target as HTMLElement | null; if (!target?.closest('.books-reader-shell') || readerMenuOpen || !target.closest('.books-reader')) return; gestureStart.current = { x: event.clientX, y: event.clientY }; }; const up = (event: PointerEvent) => { const target = event.target as HTMLElement | null; const start = gestureStart.current; gestureStart.current = null; if (!target?.closest('.books-reader-shell')) return; if (readerMenuOpen) { if (!target.closest('.books-reader-menu, .books-reader-tools, .books-reader-page-indicator')) { setReaderMenuOpen(false); setReaderPanel(null); setControlsVisible(true); } return; } if (!start || !target.closest('.books-reader')) return; if (window.getSelection()?.toString()) { setControlsVisible(true); return; } if (!isEpub && event.pointerType === 'touch' && (pdfRootRef.current?.scrollWidth || 0) > (pdfRootRef.current?.clientWidth || 0) + 4) return; const dx = event.clientX - start.x; if (Math.abs(dx) > 52) { turnPage(dx < 0 ? 1 : -1); return; } if (Math.abs(event.clientY - start.y) > 28) return; if (controlsVisible) { setControlsVisible(false); return; } if (event.clientX < window.innerWidth * .25) turnPage(-1); else if (event.clientX > window.innerWidth * .75) turnPage(1); else setControlsVisible(true); }; window.addEventListener('pointerdown', down); window.addEventListener('pointerup', up); return () => { window.removeEventListener('pointerdown', down); window.removeEventListener('pointerup', up); }; }, [selected, controlsVisible, readerMenuOpen, isEpub, pdfPage]);
  ...
  const findPdf = () => { const query = findQuery.trim().toLocaleLowerCase(); if (!query) { setFindStatus('Enter text to search this PDF.'); return; } if (!pdfDocument) return; const token = pdfSearchTokenRef.current + 1; pdfSearchTokenRef.current = token; setPdfSearchBusy(true); setPdfSearchMatches([]); setPdfSearchIndex(0); setPdfHighlightQuery(''); setFindStatus('Searching this PDF…'); void (async () => { const matches: number[] = []; let nextPage = 1; const readPage = async () => { while (nextPage <= pdfDocument.numPages) { const page = nextPage; nextPage += 1; let text = pdfTextCacheRef.current.get(page); if (text === undefined) { const content = await pdfDocument.getPage(page).then((entry) => entry.getTextContent()); text = content.items.map((item) => item.str || '').join(' ').toLocaleLowerCase(); pdfTextCacheRef.current.set(page, text); } if (text.includes(query)) matches.push(page); if (token !== pdfSearchTokenRef.current) return; } }; await Promise.all(Array.from({ length: Math.min(6, pdfDocument.numPages) }, readPage)); if (token !== pdfSearchTokenRef.current) return; matches.sort((left, right) => left - right); if (matches.length) { setPdfSearchMatches(matches); setPdfSearchIndex(0); setPdfHighlightQuery(query); changePdfPage(matches[0]); setFindStatus(`1 of ${matches.length} matching page${matches.length === 1 ? '' : 's'} · page ${matches[0]}`); } else setFindStatus('No matches in this PDF.'); })().catch(() => { if (token === pdfSearchTokenRef.current) setFindStatus('Unable to search this PDF.'); }).finally(() => { if (token === pdfSearchTokenRef.current) setPdfSearchBusy(false); }); };
  const clearEpubSearchHighlight = () => { const highlighted = epubSearchHighlightRef.current; if (highlighted) renditionRef.current?.annotations?.remove?.(highlighted, 'highlight'); epubSearchHighlightRef.current = null; };
  const showEpubSearchMatch = (match: EpubSearchMatch) => { const rendition = renditionRef.current; if (!rendition) return; clearEpubSearchHighlight(); epubSearchHighlightRef.current = match.cfi; void rendition.display(match.cfi).then(() => { if (epubSearchHighlightRef.current !== match.cfi || renditionRef.current !== rendition) return; rendition.annotations?.highlight(match.cfi, undefined, undefined, 'td-epub-search-result', { fill: '#facc15', 'fill-opacity': '.48', 'mix-blend-mode': 'multiply' }); }).catch(() => { if (epubSearchHighlightRef.current === match.cfi) epubSearchHighlightRef.current = null; }); };
  const openEpubSearchMatch = (index: number) => { if (!epubSearchMatches.length) return; const next = (index + epubSearchMatches.length) % epubSearchMatches.length; const match = epubSearchMatches[next]; setEpubSearchIndex(next); showEpubSearchMatch(match); setFindStatus(`${next + 1} of ${epubSearchMatches.length} matches${match.excerpt ? ` · ${match.excerpt}` : ''}`); };
  const findInEpub = () => { const query = findQuery.trim(); const book = epubBookRef.current; if (!query) { setFindStatus('Enter text to search this EPUB.'); return; } if (!book?.spine?.each) { setFindStatus('Search is unavailable until this EPUB finishes opening.'); return; } const sections: EpubSection[] = []; book.spine.each((section) => sections.push(section)); if (!sections.length) { setFindStatus('This EPUB does not expose searchable chapters.'); return; } const token = epubSearchTokenRef.current + 1; epubSearchTokenRef.current = token; clearEpubSearchHighlight(); setEpubSearchBusy(true); setEpubSearchMatches([]); setEpubSearchIndex(0); setFindStatus(`Searching ${sections.length} chapters…`); void (async () => { const matches: EpubSearchMatch[] = []; for (let index = 0; index < sections.length; index += 1) { const section = sections[index]; await section.load(); if (token !== epubSearchTokenRef.current) { section.unload?.(); return; } matches.push(...section.find(query)); section.unload?.(); if ((index + 1) % 8 === 0 && index + 1 < sections.length) setFindStatus(`Searching chapter ${index + 1} of ${sections.length}…`); } if (token !== epubSearchTokenRef.current) return; if (matches.length) { setEpubSearchMatches(matches); setEpubSearchIndex(0); showEpubSearchMatch(matches[0]); setFindStatus(`1 of ${matches.length} matches${matches[0].excerpt ? ` · ${matches[0].excerpt}` : ''}`); } else setFindStatus('No matches in this EPUB.'); })().catch(() => { if (token === epubSearchTokenRef.current) setFindStatus('Unable to search this EPUB.'); }).finally(() => { if (token === epubSearchTokenRef.current) setEpubSearchBusy(false); }); };
  ...
  const bookCard = (book: BookItem) => <button type="button" className="book-card" key={book.id} onClick={() => openBook(book)}><span className="book-cover book-cover-image">{book.coverUrl && <img src={book.coverUrl} alt="" loading="lazy" decoding="async" onError={(event) => { event.currentTarget.hidden = true; }} />}<BookOpenIcon /><strong className="book-cover-fallback-title">{book.title}</strong><small>{book.format}</small></span><span className="book-card-copy"><strong className="book-card-title">{book.title}</strong>{book.authors.length > 0 ? <span className="book-author">{book.authors.join(', ')}</span> : <span className="book-author">{book.publisher || 'Unknown author'}</span>}<span className="book-card-meta">{book.format}{book.pageCount ? ` · ${book.pageCount} pages` : ''}{progress[book.id]?.progress ? ` · ${Math.round(progress[book.id].progress * 100)}% read` : ''}</span></span></button>;

  if (selected) return (
    <main className="books-reader-page" onFocusCapture={() => setControlsVisible(true)}>
      <section className={`books-reader-shell${isEpub ? ' books-reader-shell-epub' : ' books-reader-shell-pdf'}`} aria-keyshortcuts="Escape Shift+Slash ArrowLeft ArrowRight">
        <button type="button" className="books-reader-reveal-control" onClick={() => { setControlsVisible(true); setReaderMenuOpen(true); }}>Show reader controls</button>
        ...
        {!isEpub && <nav className="books-reader-pagination" aria-label="PDF page navigation">
          ...
          <button type="button" className="books-reader-page-indicator" aria-expanded={readerMenuOpen} onClick={() => { setControlsVisible(true); setReaderPanel(null); setReaderMenuOpen((open) => !open); }}>Page {pdfPage}{pdfPages ? ` / ${pdfPages}` : ''}</button>
  ...
  return <main className="hub-main books-page">
    <section className="books-hero"><div><p className="eyebrow">Library books</p><h1>Your reading library.</h1><p>Browse and read available PDFs and EPUBs from one comfortable place.</p></div></section>
    <div className="books-library-controls">
      <label className="books-search"><SearchIcon /><input aria-label="Search books" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search books" /></label>
      <div className="books-filter-row" aria-label="Book library controls">
        <label>Format<select value={formatFilter} onChange={(event) => setFormatFilter(event.target.value as BookFormatFilter)}><option value="all">All formats</option><option value="pdf">PDF</option><option value="epub">EPUB</option></select></label>
        <label>Reading<select value={readingFilter} onChange={(event) => setReadingFilter(event.target.value as BookReadingFilter)}><option value="all">All books</option><option value="reading">In progress</option><option value="unread">Unread</option><option value="finished">Finished</option></select></label>
        <label>Sort<select value={bookSort} onChange={(event) => setBookSort(event.target.value as BookSort)}><option value="added">Recently added</option><option value="progress">Recently read</option><option value="title">Title</option><option value="author">Author</option></select></label>
      </div>
    </div>
