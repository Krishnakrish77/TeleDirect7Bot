...
export function BooksPage({ user }: { user: User | null }) {
  ...
  const findPdf = () => { const query = findQuery.trim().toLocaleLowerCase(); if (!query) { setFindStatus('Enter text to search this PDF.'); return; } if (!pdfDocument) return; const token = pdfSearchTokenRef.current + 1; pdfSearchTokenRef.current = token; setPdfSearchBusy(true); setFindStatus('Searching this PDF…'); void (async () => { const matches: number[] = []; for (let page = 1; page <= pdfDocument.numPages; page += 1) { const content = await pdfDocument.getPage(page).then((entry) => entry.getTextContent()); if (content.items.some((item) => (item.str || '').toLocaleLowerCase().includes(query))) matches.push(page); if (token !== pdfSearchTokenRef.current) return; } if (matches.length) { changePdfPage(matches[0]); setFindStatus(`Found ${matches.length} matching page${matches.length === 1 ? '' : 's'}; opened page ${matches[0]}.`); } else setFindStatus('No matches in this PDF.'); })().catch(() => { if (token === pdfSearchTokenRef.current) setFindStatus('Unable to search this PDF.'); }).finally(() => { if (token === pdfSearchTokenRef.current) setPdfSearchBusy(false); }); };
