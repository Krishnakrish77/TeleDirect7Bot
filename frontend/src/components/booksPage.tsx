...
export function BooksPage() {
  ...
  return <main className="hub-main books-page"><section className="books-hero"><div><p className="eyebrow">Library books</p><h1>Your bot-powered bookshelf.</h1><p>Forward PDFs or EPUBs to the bot as the admin and they will appear here, served securely from your Telegram library.</p></div></section><label className="books-search"><SearchIcon /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search books" /></label>{loading ? <p className="books-state">Loading books…</p> : error ? <p className="books-reader-error">{error}</p> : items.length ? <div className="books-grid">{items.map((book) => <article className="book-card" key={book.id}><span className="book-cover"><BookOpenIcon /><small>{book.format}</small></span><div><p className="eyebrow">{book.format} · {book.fileSizeLabel}</p><h2>{book.title}</h2><p>{book.description || book.fileName}</p><Button size="sm" onClick={() => setSelected(book)}>Read book</Button></div></article>)}</div> : <section className="books-dropzone books-empty"><BookOpenIcon /><strong>No books in your library yet</strong><span>Forward a PDF or EPUB to the bot from the admin account, then refresh this page.</span></section>}</main>;
}
