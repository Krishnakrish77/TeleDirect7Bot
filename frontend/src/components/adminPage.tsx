...
const ADMIN_JOBS: AdminJobDefinition[] = [
  ...
  {
    key: 'credits',
    label: 'TMDB detail backfill',
    description: 'Fills missing credits, ratings, runtime, certification, and topics from existing TMDB IDs.',
    action: 'backfill-credits',
    actionLabel: 'Backfill TMDB details',
    confirmMessage: 'Backfill missing TMDB detail fields from existing TMDB IDs?',
...
function MaintenancePanel({
  ...
  const actions = [
    ['backfill-credits', 'Backfill TMDB details', 'Fill missing credits, ratings, runtime, certification, and topics from existing TMDB IDs', 'Backfill missing TMDB detail fields from existing TMDB IDs?'],
  ...
  return (
    <article className={['admin-row', item.hidden ? 'hidden-row' : '', item.duplicate ? 'duplicate-row' : ''].filter(Boolean).join(' ')}>
      ...
      <div className="admin-chip-row admin-row-issues">
        ...
        {item.mediaKind !== 'audio' && (item.subtitleProbePending ? <i>Subtitles unprobed</i> : item.subtitleCount ? <i className="ok">{item.subtitleCount} subtitle{item.subtitleCount === 1 ? '' : 's'}</i> : <i className="warn">No subtitles</i>)}
        {!item.hidden && !item.duplicate && !item.missingPoster && !item.missingThumb && Boolean(item.subtitleCount || item.mediaKind === 'audio' || item.subtitleProbePending) && <i className="ok">No issues</i>}
      </div>
